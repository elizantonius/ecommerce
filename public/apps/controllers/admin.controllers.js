...
function homeController($scope, $http, helperServices, homeServices) {
  ...
  $scope.title = "Testing Home";
}

function fasilitasController(
  $scope,
  $http,
  helperServices,
  fasilitasServices,
  ...
  $scope.edit = (item) => {
    ...
    $scope.model.files = $sce.trustAsResourceUrl(
      helperServices.url + "assets/backend/img/foto/" + item.gambar
    );
    $("#tambah").modal("show");
  };
  ...
  $scope.clear = () => {
    $scope.model = {};
    $scope.model.files = $sce.trustAsResourceUrl(
      helperServices.url + "assets/backend/img/foto/" + ""
    );
  };
  ...
  $scope.showFoto = (item) => {
    $scope.$applyAsync((x) => {
      $scope.files = $sce.trustAsResourceUrl(
        helperServices.url + "assets/backend/img/foto/" + item.gambar
      );
    });
    $("#modelId").modal("show");
  ...
  paketServices.get().then((res) => {
    ...
    menuServices.get().then((x) => {
      ...
      $scope.menus.forEach((element) => {
        element.foto = $sce.trustAsResourceUrl(
          helperServices.url + "assets/backend/img/makanan/" + element.foto
        );
      });
    });
  });
...
function pegawaiController(
  ...
  $scope.showFoto = (item) => {
    $scope.$applyAsync((x) => {
      $scope.files = $sce.trustAsResourceUrl(
        helperServices.url + "assets/backend/img/foto/" + item.foto
      );
    });
    $("#modelId").modal("show");
  };
...
function jadwalController(
  ...
  $scope.grouptanggal = (data) => {
    ...
    newArray.forEach((element) => {
      ...
      element.color =
        item.length > 0 && item.length < 5
          ? "green"
          : item.length > 4 && item.length < 10
          ? "yellow"
          : item.length > 9 && item.length < 15
          ? "orange"
          : item.length == 15
          ? "#dc3545"
          : "";
...
function pesananController(
  $scope,
  helperServices,
  pesananAdminServices,
  message,
  $sce
) {
  ...
  $scope.numm = () => {
    return $scope.$emit(
      "send",
      $scope.datas.pesanan.filter(
        (x) => x.status_bayar == "0" && x.pembayaran.length > 0
      ).length
    );
  };
  ...
  $scope.edit = (item) => {
    ...
    $scope.model.pembayaran.forEach((element) => {
      element.bukti = $sce.trustAsResourceUrl(
        helperServices.url + "assets/backend/img/pembayaran/" + element.bukti
      );
  ...
  $scope.save = () => {
    message
      .dialog("Ingin mengkonfirmasi pembayaran?", "Ya", "Tidak")
      .then((x) => {
...
function bokingController(
  ...
  $scope.save = () => {
    message
      .dialog("Data setelah di simpan tidak dapat diubah!", "Ya", "Tidak")
      .then((x) => {
      ...
      laporanServices.get($scope.model).then((x) => {
        ...
        $scope.datas.forEach((element, key) => {
          if (element.status_bayar != "0" && element.pembayaran.length > 0) {
            ...
            $scope.html +=
              ...
              element.pembayaran.length +
              "'>" +
              element.tanggal_pakai +
              "</td>" +
              "<td rowspan='" +
              element.pembayaran.length +
              "'>" +
              (element.status_bayar == "0"
                ? "Belum Bayar"
                : element.status_bayar == "1"
                ? "Panjar"
                : "Lunas") +
              "</td>" +
              "<td>" +
              element.pembayaran[0].tanggalbayar +
              "</td>" +
              "<td class='text-right'>" +
              new Intl.NumberFormat("id-IN", {
                style: "currency",
                currency: "IDR",
              }).format(element.pembayaran[0].nominal) +
              "</td>" +
              ...
              new Intl.NumberFormat("id-IN", {
                style: "currency",
                currency: "IDR",
              }).format(element.total_bayar.nominal) +
              "</td>" +
              "<td class='text-right' rowspan='" +
              element.pembayaran.length +
              "'>" +
              ...
              new Intl.NumberFormat("id-IN", {
                ...
                parseFloat(element.tagihan) -
                  parseFloat(element.total_bayar.nominal)
              ) +
              "</td>" +
              "</tr>";
        ...
        $scope.html +=
          ...
          new Intl.NumberFormat("id-IN", {
            style: "currency",
            currency: "IDR",
          }).format(totalTagihan - totalBayar) +
          "</td>" +
          "</tr>" +
          "</tfoot>" +
          "</table>";
